...
import { fetchSubmissionsByProblemId } from '@/redux/slices/submissionSlice';
...
const ProblemSubmission: React.FC = () => {
  ...
  const { problem, loading: problemLoading, error: problemError } = useSelector(
    (state: RootState) => state.problems
  );
  const { submissions, submissionsStatus, submissionsError } = useSelector(
  ...
  useEffect(() => {
    if (problemId) {
      ...
      dispatch(fetchSubmissionsByProblemId(problemId));
    }
  }, [dispatch, problemId]);
  ...
  return (
    <div className="px-10">
      ...
      <div className="flex justify-end gap-4 mt-4">
        ...
        <Link href={`/dashboard/problems/problemsubmission/submissionform?id=${problemId}`}>
          <Button
            variant="outline"
